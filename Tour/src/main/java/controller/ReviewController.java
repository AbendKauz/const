...
public class ReviewController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/rList.re")) {
			...
			int result = modifyReview(request, response);
			if(result == 0) {
				JSFunction.alertLocation(response, "리뷰 수정에 실패하셨습니다.", "history.back(-1);");
			}else {
				JSFunction.alertLocation(response, "리뷰 수정에 성공하셨습니다.", "/rList.re");
			}
		} else if(command.equals("/delReview.re")) {
	...
	// 리뷰 수정
	private int modifyReview(HttpServletRequest request, HttpServletResponse response) {
		// 파일 업로드 관련
		String saveDirectory = request.getServletContext().getRealPath("/uploads");
		// 업로드 디렉토리의 물리적인 경로 확인
		ServletContext application = getServletContext();
		...
		dto.setRbno(Integer.parseInt(mr.getParameter("rbno")));
		dto.setRid(mr.getParameter("uid"));
		dto.setRtitle(mr.getParameter("title"));
		dto.setRcont(mr.getParameter("content"));
		dto.setRegDate(today.format(date.getTime()));
		...
		int result = dao.modifyReview(dto);
		return result;
	}
	
	// 리뷰 삭제
	private void delReview(HttpServletRequest request, HttpServletResponse response) {
		int rbno = Integer.parseInt(request.getParameter("rbno"));
		
		ReviewDAO dao = ReviewDAO.getInstance();
		int result = dao.deleteReview(rbno);
		if(result == 0) {
			JSFunction.alertLocation(response, "리뷰 삭제에 실패했습니다.", "/rList.re");
		} else {
			JSFunction.alertLocation(response, "리뷰가 삭제되었습니다.", "/rList.re");
		}
	}
